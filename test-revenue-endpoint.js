...
async function testRevenueEndpoint() {
  try {
    ...
    try {
      const revenueResponse = await axios.get('https://api.householdplanetkenya.co.ke/admin/analytics/revenue?period=monthly');
      ...
      if (error.response?.status === 401) {
        ...
        try {
          ...
          // Now test with auth
          const authenticatedResponse = await axios.get('https://api.householdplanetkenya.co.ke/admin/analytics/revenue?period=monthly', {
            headers: {
