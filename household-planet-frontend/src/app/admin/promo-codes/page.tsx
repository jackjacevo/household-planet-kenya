...
export default function PromoCodesPage() {
  ...
  const fetchPromoCodes = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/promo-codes?search=${searchTerm}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingCode 
        ? `${process.env.NEXT_PUBLIC_API_URL}/promo-codes/${editingCode.id}`
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/promo-codes/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
