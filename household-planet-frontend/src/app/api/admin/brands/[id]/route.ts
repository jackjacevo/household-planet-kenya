...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const backendUrl = `${BACKEND_URL}/api/admin/brands/${params.id}`;
    ...
    console.error('Admin brand GET API error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch brand', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error('Admin brand PUT API error:', error);
    return NextResponse.json(
