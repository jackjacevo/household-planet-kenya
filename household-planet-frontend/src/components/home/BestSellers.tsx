...
export function BestSellers() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const data = await api.getProducts({ limit: 5, featured: true }) as any;
              ...
              <div key={product.id} className="product-card bg-white rounded-lg overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition duration-300">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center mb-2">
                    ...
                    <span className="text-xs text-gray-500 ml-1">({product.totalReviews || Math.floor(Math.random() * 50) + 10})</span>
                  </div>
