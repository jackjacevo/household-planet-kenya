...
async function checkPaymentData() {
  ...
  try {
    ...
    if (completedPayments.length > 0) {
      ...
      console.log('\nNo completed payments found. Let\'s create a test payment...');
      
      // Find any order and create a completed payment for it
      const anyOrder = await prisma.order.findFirst({
        where: { status: { not: 'CANCELLED' } }
      });
      
      if (anyOrder) {
        console.log(`Creating test payment for order ${anyOrder.orderNumber}...`);
        
        const testPayment = await prisma.paymentTransaction.create({
          data: {
            orderId: anyOrder.id,
            checkoutRequestId: `test_${Date.now()}`,
            phoneNumber: '254700000000',
            amount: anyOrder.total,
            status: 'COMPLETED',
            provider: 'MPESA',
        ...
        console.log(`Test payment created with ID: ${testPayment.id}`);
        ...
        await prisma.order.update({
          ...
          data: { 
            paymentStatus: 'PAID',
            status: 'DELIVERED' // Set to delivered so receipt can be generated
          }
        });
        
        console.log(`Order ${anyOrder.orderNumber} updated to DELIVERED with PAID status`);
      }
    }
